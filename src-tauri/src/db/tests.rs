...
/// 创建测试用的消息数据
fn create_test_message(conversation_id: i64, message_type: &str, content: &str, parent_id: Option<i64>, generation_group_id: Option<String>) -> Message {
    Message {
        id: 0,
        parent_id,
        conversation_id,
...
mod message_repository_tests {
    ...
    fn test_list_messages_by_conversation_id() {
        ...
        // 创建多条消息
        let messages = vec![
            create_test_message(conversation.id, "user", "Message 1", None, Some(Uuid::new_v4().to_string())),
            create_test_message(conversation.id, "assistant", "Message 2", None, Some(Uuid::new_v4().to_string())),
            create_test_message(conversation.id, "user", "Message 3", None, Some(Uuid::new_v4().to_string())),
        ];
        ...
        // 验证消息内容
        let contents: Vec<String> = retrieved_messages.iter()
