...
use rig::{completion::Chat, providers::deepseek as rig_deepseek, streaming::StreamingChat};
...
impl ModelProvider for DeepSeekProvider {
    ...
    fn chat(
        ...
        Box::pin(async move {
            ...
            // DeepSeek 默认使用其官方 endpoint，也支持自定义
            let default_endpoint = "https://api.deepseek.com";
            let endpoint_opt = config_map.get("endpoint");
            let client = if let Some(ep) = endpoint_opt {
                rig_deepseek::Client::from_url(api_key, ep.trim_end_matches('/'))
            } else {
                rig_deepseek::Client::from_url(api_key, default_endpoint)
            };
    ...
    fn models(&self) -> futures::future::BoxFuture<'static, Result<Vec<LlmModel>>> {
        ...
        Box::pin(async move {
            ...
            let default_endpoint = &"https://api.deepseek.com/v1".to_string();
            ...
            println!("DeepSeek models endpoint : {}", url);
            ...
            for model in models_response.data {
                let llm_model = LlmModel {
                    ...
                    name: model.id.clone(),
                    llm_provider_id: 30, // DeepSeek provider id
                    code: model.id.clone(),
                    description: format!(
                    ...
                    vision_support: model.id.contains("vision") || model.id.contains("v"), // 假设包含这些关键词的支持视觉
                    audio_support: false, // DeepSeek 目前不支持音频
                    video_support: false, // DeepSeek 目前不支持视频
                };
