    ...
    vec![BuiltinTemplateInfo {
        ...
        required_envs: vec![
            BuiltinTemplateEnvVar {
                ...
                label: "浏览器类型".into(),
                required: false,
                tip: Some("选择要使用的浏览器类型，默认使用 Chrome，如果不可用则降级为 Edge".into()),
            ...
            BuiltinTemplateEnvVar {
                ...
                label: "搜索引擎".into(),
                required: true,
                tip: Some("选择要使用的搜索引擎，默认使用 Google，如果不可用则降级为 Bing".into()),
                field_type: "select".into(),
            ...
            BuiltinTemplateEnvVar {
                ...
                label: "浏览器数据目录".into(),
                required: false,
                tip: Some("指向用户日常使用的浏览器 profile 目录，可以共享登录状态和配置".into()),
            ...
            BuiltinTemplateEnvVar {
                ...
                label: "用户代理".into(),
                required: false,
                tip: Some("自定义浏览器的 User-Agent 字符串，用于模拟不同设备或浏览器".into()),
                field_type: "text".into(),
                default_value: None,
                placeholder: Some("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36".into()),
                options: None,
            },
            BuiltinTemplateEnvVar {
                key: "BYPASS_CSP".into(),
                label: "绕过内容安全策略".into(),
                required: false,
                tip: Some("绕过网页的内容安全策略限制，可能有助于访问某些受限制的页面".into()),
                field_type: "boolean".into(),
            ...
            BuiltinTemplateEnvVar {
                ...
                label: "等待元素选择器".into(),
                required: false,
                tip: Some("等待页面指定元素加载完成的 CSS 选择器，多个选择器用逗号分隔".into()),
                field_type: "text".into(),
                default_value: None,
