use crate::db::conversation_db::{Message, Conversation, Repository};
use chrono::Utc;
use uuid::Uuid;
...
async fn test_message_filtering_logic() {
    ...
    // 测试用户消息重发的过滤逻辑
    let user_regenerate_messages: Vec<_> = messages
        .iter()
        .filter(|m| m.0 <= user_msg_id)
        .collect();
    ...
    // 测试AI消息重发的过滤逻辑
    let ai_regenerate_messages: Vec<_> = messages
        .iter()
        .filter(|m| m.0 < ai_msg_id)
        .collect();
