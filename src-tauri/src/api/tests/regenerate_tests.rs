...
use crate::db::conversation_db::{ConversationDatabase, Message, MessageRepository, Repository, Conversation, MessageAttachment};
use crate::api::ai_api::{regenerate_ai, AiResponse};
...
async fn test_regenerate_user_message_creates_new_generation() {
    ...
    let system_msg = create_test_message(
        1, None, 1, "system", "You are a helpful assistant", 
        None, None, None, None, base_time
    );
    
    let user_msg = create_test_message(
        2, None, 1, "user", "What is 2+2?", 
        Some(1), Some("gpt-4".to_string()), None, None,
        base_time + chrono::Duration::seconds(1)
    );
    
    let reasoning_msg = create_test_message(
        3, None, 1, "reasoning", "Let me calculate 2+2", 
        Some(1), Some("gpt-4".to_string()), Some(original_group_id.clone()), None,
        base_time + chrono::Duration::seconds(2)
    );
    
    let response_msg = create_test_message(
        4, None, 1, "response", "2+2 equals 4", 
        Some(1), Some("gpt-4".to_string()), Some(original_group_id.clone()), None,
...
async fn test_message_version_grouping() {
    ...
    let messages = vec![
        ...
        create_test_message(2, None, 1, "reasoning", "Thinking v1", Some(1), Some("gpt-4".to_string()), Some(group_id_1.clone()), None, base_time + chrono::Duration::seconds(1)),
        create_test_message(3, None, 1, "response", "Response v1", Some(1), Some("gpt-4".to_string()), Some(group_id_1.clone()), None, base_time + chrono::Duration::seconds(2)),
        create_test_message(4, Some(2), 1, "reasoning", "Thinking v2", Some(1), Some("gpt-4".to_string()), Some(group_id_2.clone()), None, base_time + chrono::Duration::seconds(3)),
        create_test_message(5, Some(3), 1, "response", "Response v2", Some(1), Some("gpt-4".to_string()), Some(group_id_2.clone()), None, base_time + chrono::Duration::seconds(4)),
    ];
...
async fn test_deprecated_assistant_message_type() {
    ...
    let messages = vec![
        create_test_message(1, None, 1, "user", "Hello", None, None, None, None, base_time),
        create_test_message(2, None, 1, "assistant", "Deprecated response", Some(1), Some("gpt-4".to_string()), None, None, base_time + chrono::Duration::seconds(1)),
        create_test_message(3, None, 1, "response", "New response", Some(1), Some("gpt-4".to_string()), Some(Uuid::new_v4().to_string()), None, base_time + chrono::Duration::seconds(2)),
    ];
    
    // 验证废弃消息类型检测
    let deprecated_messages: Vec<&Message> = messages.iter()
...
async fn test_llm_model_fields_in_messages() {
    ...
    let reasoning_msg = create_test_message(
        1, None, 1, "reasoning", "Thinking...", 
        Some(42), Some("gpt-4-turbo".to_string()), Some(group_id.clone()), None, base_time
    );
    
    let response_msg = create_test_message(
        2, None, 1, "response", "Here's my answer", 
        Some(42), Some("gpt-4-turbo".to_string()), Some(group_id.clone()), None, base_time + chrono::Duration::seconds(1)
    );
    
    let user_msg = create_test_message(
        3, None, 1, "user", "Hello", 
        None, None, None, None, base_time + chrono::Duration::seconds(2)
    );
...
async fn test_generation_group_id_consistency() {
    ...
    let reasoning_msg = create_test_message(
        1, None, 1, "reasoning", "Let me think about this...", 
        Some(1), Some("gpt-4".to_string()), Some(group_id.clone()), None, base_time
    );
    
    let response_msg = create_test_message(
        2, None, 1, "response", "Based on my reasoning, the answer is...", 
        Some(1), Some("gpt-4".to_string()), Some(group_id.clone()), None, base_time + chrono::Duration::seconds(1)
    );
...
async fn test_parent_child_relationship_logic() {
    ...
    let original_response = create_test_message(
        1, None, 1, "response", "Original answer", 
        Some(1), Some("gpt-4".to_string()), Some(group_id_1.clone()), None, base_time
    );
    
    let regenerated_response = create_test_message(
        2, Some(1), 1, "response", "Regenerated answer", 
        Some(1), Some("gpt-4".to_string()), Some(group_id_2.clone()), None, base_time + chrono::Duration::seconds(1)
    );
...
async fn test_message_filtering_for_regenerate() {
    ...
    // 创建完整的对话历史
    let messages = vec![
        create_test_message(1, None, 1, "system", "You are helpful", None, None, None, None, base_time),
        create_test_message(2, None, 1, "user", "Question 1", None, None, None, None, base_time + chrono::Duration::seconds(1)),
        create_test_message(3, None, 1, "reasoning", "Thinking...", Some(1), Some("gpt-4".to_string()), Some(group_id.clone()), None, base_time + chrono::Duration::seconds(2)),
        create_test_message(4, None, 1, "response", "Answer 1", Some(1), Some("gpt-4".to_string()), Some(group_id.clone()), None, base_time + chrono::Duration::seconds(3)),
        create_test_message(5, None, 1, "user", "Question 2", None, None, None, None, base_time + chrono::Duration::seconds(4)),
        create_test_message(6, None, 1, "response", "Answer 2", Some(1), Some("gpt-4".to_string()), Some(Uuid::new_v4().to_string()), None, base_time + chrono::Duration::seconds(5)),
    ];
    ...
    let filtered_for_user: Vec<&Message> = messages.iter()
