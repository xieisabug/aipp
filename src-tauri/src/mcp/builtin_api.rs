...
// Re-export everything from the consolidated MCP module location
pub use crate::mcp::builtin_mcp::{
    add_or_update_aipp_builtin_server, execute_aipp_builtin_tool, get_builtin_tools_for_command,
    is_builtin_command as is_aipp_builtin_command, list_aipp_builtin_templates,
    BuiltinTemplateEnvVar, BuiltinTemplateInfo, BuiltinToolInfo, SearchHandler,
};
...
impl BuiltinSearchHandler {
    ...
    pub async fn search_web(&self, query: &str) -> Result<serde_json::Value, String> {
        use crate::mcp::builtin_mcp::search::types::{
            SearchRequest, SearchResponse, SearchResultType,
        };
        ...
        match handler.search_web_with_type(request).await {
            Ok(SearchResponse::Html { html_content, .. }) => Ok(serde_json::json!({
                "query": query,
