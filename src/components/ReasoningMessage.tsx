...
const ReasoningMessage = React.memo(({
    ...
    // 性能监控
    usePerformanceMonitor('ReasoningMessage', [
        message.id,
        message.start_time,
        message.finish_time,
        streamEvent?.is_done,
    ...
    useEffect(() => {
        ...
        const getUpdateInterval = () => {
            ...
            const elapsed = Date.now() - new Date(message.start_time).getTime();
            // 思考超过1分钟后，每5秒更新一次
    ...
    const formatStatusText = useCallback((baseText: string) => {
        return thinkingTime ? `${baseText}(${baseText === '思考中...' ? '已' : ''}思考 ${thinkingTime})` : baseText;
    }, [thinkingTime]);
    ...
    return (
        <div className="my-2 p-3 bg-gray-50 border-l-4 border-gray-400 rounded-r-lg max-w-[80%]">
            ...
            <div className="text-sm text-gray-600 whitespace-pre-wrap font-mono">
                {isThinking && contentLines.hasMoreThanThreeLines && !isReasoningExpanded ? (
                    <>
